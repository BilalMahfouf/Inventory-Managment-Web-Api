...
async function updateOrderPayment(id, payload = {}) {
    try {
        const normalizedPayload = {
            amount: toFiniteNumber(payload?.amount, 0),
            paymentStatus: toFiniteNumber(payload?.paymentStatus, 1),
        };

        const { data } = await api.patch(`${URL}/${id}/payment`, normalizedPayload);
        return { success: true, data };
    } catch (error) {
        console.error(error);
        return {
            success: false,
            error: extractErrorMessage(error, 'Failed to update order payment'),
        };
    }
}
