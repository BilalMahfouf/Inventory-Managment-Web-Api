 ...
 * @param {Array<string>} props.actions - Optional action override list
 * @param {'default'|'compact'} props.mode - Visual rendering mode
 */
const OrderActionBar = ({
  ...
  const orderStatus = order?.status ?? order?.salesStatus;

  const availableActions = useMemo(() => {
    if (!orderStatus) return [];

    const transitions = STATUS_TRANSITIONS[orderStatus] || [];
    if (!Array.isArray(actions)) {
      return transitions;
    }

    return actions.filter(action => transitions.includes(action));
  }, [actions, orderStatus]);
        ...
        {availableActions.map(action => {
          ...
          return (
            <Button
              ...
              loading={
                transition.isPending && transition.variables?.action === action
              }
              className={
                mode === 'compact'
                  ? 'min-h-8 px-2 py-1 text-xs font-medium rounded-md'
                  : ''
              }
            >
              {t(config.labelKey)}
            ...
            <div className='p-6 space-y-4'>
              ...
              <div>
                <label className='block text-sm font-medium text-gray-700 mb-1'>
                  {t(
                    i18nKeyContainer.sales.orders.confirmDialog.trackingNumber
                  )}{' '}
